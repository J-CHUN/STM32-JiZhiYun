...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLEDSwitch)
    {
        info->event[info->num] = EVENT_LEDSwitch;
        info->num++;
        dataPoints->valueLEDSwitch = gizStandardDecompressionValue(LEDSwitch_BYTEOFFSET,LEDSwitch_BITOFFSET,LEDSwitch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLEDSwitch != cur->valueLEDSwitch)
    {
        GIZWITS_LOG("valueLEDSwitch Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LEDSwitch_BYTEOFFSET,LEDSwitch_BITOFFSET,LEDSwitch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLEDSwitch);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));



    devStatusPtr->valueLight = exchangeWord(gizY2XFloat(Light_RATIO,  Light_ADDITION, dataPoints->valueLight)); 
